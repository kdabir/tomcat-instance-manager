...
rem can copy tomcat6 & tomcat6w exe as service name in current dir but this wont be much useful
...
if not exist "%CATALINA_HOME%\bin\tomcat6.exe" goto home-not-valid
if not exist "%CATALINA_HOME%\bin\tomcat6w.exe" goto home-not-valid
...
set TOMCATEXE=%CATALINA_HOME%\bin\tomcat6.exe
set TOMCAT_SVC_MGR=%CATALINA_HOME%\bin\tomcat6w.exe
